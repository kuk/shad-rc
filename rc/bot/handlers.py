...
- Привет, бот random  дал твой контакт. Когда удобно встретиться/созвониться на этой неделе? Могу в будни после 17.
...
MATCH_WORLDWIDE_TEXT = 'Бот выберет случайного собеседника среди всех участников.'
...
def match_city_text(city, cities):
    ...
    if count <= 1:
        ...
        return f'''Пометил, что хочешь собеседника из города "{city}". Ты пока единственный участник оттуда.

Проверь, что написал название города, как остальные участники. Например, "Санкт-Петербург" вместо "Питер", "Париж" вместо "Paris". Список городов: {', '.join(sorted(counts))}'''
